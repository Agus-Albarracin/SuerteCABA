...
const [bestFinancesData, setBestFinancesData] = useState(() => {
  const savedBestData = localStorage.getItem('bestFinancesData');
  return savedBestData ? JSON.parse(savedBestData) : []; // Cargar datos guardados
});
...
useEffect(() => {
  ...
  localStorage.setItem('bestFinancesData', JSON.stringify(bestFinancesData)); // Guardar bestFinancesData
}, [dailyProfitsChartData, totalDeposits, totalWithdrawals, bestFinancesData]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Probar cada llamada individualmente
        const responseNetwin = await axiosD.post('/getFinancesAllUser', { supervisor: user._id });
        // console.log("Netwin response:", responseNetwin.data);
        
        const responseBestFinances = await axiosD.post('/getBestFinancesUser', { supervisor: user._id });
        // console.log("Best Finances response:", responseBestFinances.data);
        
        // const responseDatosFinancieros = await axiosD.post('/getFin', { _id: user._id });
        // console.log("Datos Financieros response:", responseDatosFinancieros.data);
...
const handleFinalAccept = () => {
  ...
  axiosD.post("/loadSuper", { amount, login })
    .then((response) => {
  ...
  NetWin del Mes Pasado: ${previousMonthWin ? previousMonthWin.toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '00.00'}
</h3>
<h3 style={{ color: 'black' }}>
  NetWin del Mes Actual: ${currentMonthWin.toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
</h3>

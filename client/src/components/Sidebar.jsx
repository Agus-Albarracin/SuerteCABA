...
//#endregion
  ...
  return (
    <Container isOpen={sidebarOpen} themeUse={theme}>
      ...
      {sidebarOpen && responsiveMode ? (

      <button
        className={`Sidebarbutton ${responsiveMode && !sidebarOpen ? 'responsive-menu-button' : 'Sidebarbutton'}`}
            ...
            {subLinks &&
              subLinks.map(({ icon, label, to }) => (
                <div className={`SubLinkContainer ${(!sidebarOpen && 'responsive-menu')}`} key={label}>
                  ...
                  >
                    <div className="Linkicon" >{icon}</div>
                    {sidebarOpen && <span onClick={() => {handleSidebarClose(); }}>{label}</span>}
                  </NavLink>
                </div>
              ))}
          </div>
...
@media (max-width: 480px) {
  ...
  .LinkContainer .Links .Linkicon svg {
    display: none; 
  }
  
  .SubLinkContainer .Links .Linkicon svg {
    display: none; 
  }
}
`;
